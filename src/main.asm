...
TRACE_SEQ_PTR_RAW EQU $FFF094
TRACE_SEQ_PTR_RES EQU $FFF098
TRACE_SEQ_INDEX EQU $FFF09C
TRACE_SEQ_BYTE EQU $FFF09E
TRACE_SEQ_SOURCE EQU $FFF0A0
TRACE_PPU_INDEX EQU $FFF0A2
TRACE_PPU_PTR_RAW EQU $FFF0A4
TRACE_PPU_PTR_RES EQU $FFF0A8
TRACE_PPU_ROM_REC_HI EQU $FFF0B6
TRACE_PPU_ROM_REC_LO EQU $FFF0B7
TRACE_PPU_ROM_REC_CTRL EQU $FFF0B8
TRACE_PPU_ROM_REC_DATA EQU $FFF0BA
...
READ_JOYPAD:
    ...
    beq     .read_hardware
    moveq   #0,D0
    move.b  (JOYPAD_OVERRIDE_P1).l,D0
    move.b  D0,($FF0000+$00F0)
    rts
.read_hardware:
    ; Strobe high
    move.b  #$40,($A10003)
...
CHR_SHADOW_LOAD:
    ; Keep emulated PPU pattern reads in sync with the startup VRAM upload.
    movea.l #ZELDA_CHR_2BPP,A0
    movea.l #$00FFC000,A1
    move.w  #(8192/4)-1,D0
.chr_shadow_loop:
    move.l  (A0)+,(A1)+
    dbra    D0,.chr_shadow_loop
    rts
...
; ?.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.?
    ...
    INCLUDE "vdp_native.asm"       ; VDP_ALT: Native Genesis rendering (no emulation layer)
...
; ?.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.??.?
    EVEN                            ; force word alignment for MOVE.L copy loop
ZELDA_TILES_4BPP:
...
ZELDA_CHR_2BPP:
    ...
    INCBIN  "..\artifacts\nes_original_build\incbin\chr\01___8DB4_96B3.chr"
    INCBIN  "..\artifacts\nes_original_build\incbin\chr\02___877F_8E7E.chr"
    INCBIN  "..\artifacts\nes_original_build\incbin\chr\01___96B4_9ED3.chr"
