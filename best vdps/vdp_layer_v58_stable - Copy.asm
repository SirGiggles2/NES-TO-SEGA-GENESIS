...
; ── Internal state variables (Genesis RAM) ──
...
; NES zero-page mirrors also watched by engine/diag
...
VDP_INIT:
    ...
    move.w  #$8134,($C00004)    ; Mode reg 2 base setup during init

    ; Set up name tables
    move.w  #$8230,($C00004)    ; Plane A at VRAM $C000
    move.w  #$8307,($C00004)    ; Window at VRAM $E000
    move.w  #$8407,($C00004)    ; Plane B at VRAM $E000
    move.w  #$856C,($C00004)    ; Sprite table at $D800

    ; Colors / scroll
    move.w  #$8700,($C00004)
    move.w  #$8B00,($C00004)
    move.w  #$8C81,($C00004)    ; H40 mode
    move.w  #$8D2F,($C00004)    ; HScroll table at $BC00
    move.w  #$8F02,($C00004)    ; Auto-inc = 2
    move.w  #$9001,($C00004)    ; Scroll size 64x32
    move.w  #$9100,($C00004)
    move.w  #$9200,($C00004)
    ...
    ; Display ON, VINT OFF until game explicitly enables NMI via $2000 bit 7
...
; ═══════════════════════════════════════════════════════════════
PPU_WRITE_2000:
    ori.b   #$80,D0                  ; force NES NMI-enable bit to stay on
    move.b  D0,(PPUCTRL_SHADOW).l    ; save Genesis shadow copy
    move.b  D0,(RAM_FOR_2000).l      ; save NES mirror copy

    ; Bit 7 = NMI enable -> controls VBlank IRQ (reg1 bit 5)
    ; Preserve display bit 6 and only toggle bit 5 from the NES shadow write.
...
PPU_READ_2002:
    ...
    move.w  D0,D3
    clr.b   D0
    btst    #3,D3                 ; Genesis VBlank flag is bit 3
    beq     .no_vblank
    bset    #7,D0                 ; Return NES VBlank bit
.no_vblank:
    clr.b   (PPUADDR_LATCH).l
    clr.b   (PPUSCROLL_LATCH).l
    rts
...
PPU_WRITE_2005:
    ...
    ; First write = X scroll. Update both Genesis shadow and NES ZP mirror.
...
.write_y:
    ; Second write = Y scroll. Update both Genesis shadow and NES ZP mirror.
...
; ═══════════════════════════════════════════════════════════════
; VDP_VBLANK_HANDLER
; Temporary bring-up handler:
; - acks VBlank
; - updates the RAM state Zelda is waiting on
; - forces rendering enabled through the real $2001 write path
; - forces one complete $2005 scroll write pair through the real path
; - DOES NOT call translated vec_0x01E494_NMI yet
; ═══════════════════════════════════════════════════════════════
VDP_VBLANK_HANDLER:
    movem.l D0-D7/A0-A6,-(A7)

    move.w  ($C00004),D0           ; acknowledge VBlank interrupt

    addq.b  #1,($FF0015)           ; frame counter / NMI completion
    move.b  #1,($FF0011)           ; screen_ready
    move.b  #$3F,($FF0300)         ; PPU buffer complete status
    move.b  #$FF,($FF0302)         ; terminator byte expected by diag

    ; TEMP bring-up: force PPUMASK/render enable through real write path
    moveq   #$1E,D0
    bsr     PPU_WRITE_2001

    ; TEMP bring-up: force one full scroll commit through the real $2005 path.
    ; Use a harmless 1/1 pair so both X and Y mirrors visibly change.
    moveq   #1,D0
    bsr     PPU_WRITE_2005
    moveq   #1,D0
    bsr     PPU_WRITE_2005

    ; Removed redundant manual re-apply of scroll shadows here.
    ; PPU_WRITE_2005 already commits both X and Y to the VDP.

    ; TEMP: skip translated NMI body until crash path is isolated
