...
$SCRIPT = "$PROJECT\diag\scripts\zelda_cram_brightness_probe.lua"
$TEMP_SCRIPT = "$PROJECT\diag\scripts\zelda_cram_brightness_probe.runtime.lua"
$ROM_BASENAME = [System.IO.Path]::GetFileNameWithoutExtension($Rom)
$ROM_PATH = "$BUILD_DIR\$ROM_BASENAME.md"
$OUTPUT_PATH = "$PROJECT\diag\reports\cram_brightness_probe_$ROM_BASENAME.txt"
...
try {
    Write-Host "=== CRAM BRIGHTNESS PROBE ===" -ForegroundColor Cyan
    Write-Host "ROM:    $ROM_BASENAME"
    Write-Host "Output: $OUTPUT_PATH"
    Write-Host ""
    Write-Host "NOTE: Script captures title automatically." -ForegroundColor Yellow
    Write-Host "      Navigate to file-select and start a game for full capture." -ForegroundColor Yellow
    Write-Host ""
