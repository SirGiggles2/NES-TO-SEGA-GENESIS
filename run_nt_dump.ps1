...
try {
    if (Test-Path $LOCK_PATH) {
        throw "NT dump runner is already active. Remove $LOCK_PATH if the previous run crashed."
    }
    ...
    if (-not $Completed -and -not (Test-Path $OutPath)) {
        throw "NT dump did not produce $OutPath"
    }
}
finally {
