...
for frame = 0, MAX_FRAMES do
    ...
    local periodic = (handler2_active and (nmi_count - last_log_nmi) >= 50)

    if state_changed or periodic then
        log(string.format("f=%05d frm=%3d rdy=%d ad=%d ph=%d timer=%3d sy=$%02X 2000=$%02X 5C=%d 0415=%d 042E=%02X 0419=%02X nmi=%d cfr=%d",
            frame, frm, rdy, auto_demo, phase, timer, scroll_Y, r2000, ram_005C, ram_0415, ram_042E, ram_0419, nmi_count, consecutive_frozen))
    ...
    if handler2_active and consecutive_frozen >= 10 and not freeze_reported then
        ...
        log(string.format("  ram_042E=$%02X ram_0419=$%02X ptr=$%02X%02X",
            ram_042E, ram_0419, r8(0xFF0017), r8(0xFF0018)))
    end
