...
local ROM_VERSION  = "zelda_v378"   -- patched by PS launcher
local MAX_FRAMES   = 12000    -- 200s of emulated time (enough for curtain + script4)

local OUT_DIR  = "C:\\Users\\Jake Diggity\\Documents\\GitHub\\NES-TO-SEGA-GENESIS\\diag\\reports\\"
local OUT_PATH = OUT_DIR .. "script3_diag_zelda_" .. ROM_VERSION .. ".txt"
...
log("")
log("Frame | Scr | Sub | Ready | PpuIdx | 007C | 007D | Rendr | Timer | Notes")
log(string.rep("-", 90))
...
while frame_num < MAX_FRAMES do
    ...
    -- Track stalls: same script+subscript for too long
    ...
    -- Inside script 3-4: log all state changes
    ...
    if should_log then
        log(string.format("%5d | %3d | %3d | %5d | 0x%02X   | %4d | %4d | %5d | %5d | %s",
    ...
    -- Success: gameplay reached (script >= 5)
    ...
    -- Hang detection: ANY script/subscript stuck for 3000 frames
    if stall_count > 3000 and script >= 3 then
        log("")
        log(string.format("*** STUCK: script=%d sub=%d for %d frames (ready=%d ppu=0x%02X 007C=%d timer=%d)",
            script, subscript, stall_count, ready, ppu_idx, r007c, timer))
        if script == 3 and subscript <= 2 then
            log("   -> PPU buffer hang: ppu_load_index not clearing (unhandled buffer in write_buffer_to_ppu)")
        elseif script == 3 and ready == 1 then
            log("   -> Secondary handler stuck: curtain not finishing (check sub_bat_7248 / ram_007C)")
...
log("")
log("=== SUMMARY ===")
log(string.format("Final: script=%d sub=%d ready=%d ppu_idx=0x%02X 007C=%d timer=%d",
