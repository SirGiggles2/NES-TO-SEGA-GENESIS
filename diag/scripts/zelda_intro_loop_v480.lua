-- zelda_intro_loop_v480.lua
-- Tests v480 ROM: verifies NMI no longer freezes, and monitors intro loop cycle.
-- Expected: title display (~512 NMI ticks) → fade → intro scroll → fade → title (repeat)

local ROM_VERSION = "zelda_v480"
local MAX_FRAMES = 6000  -- enough for one full intro cycle (~4800 NMI ticks on NES)
...
for frame = 0, MAX_FRAMES do
    ...
    if handler2_active and consecutive_frozen >= 10 and not freeze_reported then
        ...
        log(string.format("*** FREEZE DETECTED at f=%d frm=%d nmi=%d PC=%06X SR=%04X ***",
            frame, frm, nmi_count, pc, sr))
    end
...
log("")
log(string.format("=== FINAL STATE: f=%d nmi=%d frm=%d frozen=%s ===",
