...
$SCRIPT = "$PROJECT\diag\scripts\zelda_title_vsram_probe.lua"
$TEMP_SCRIPT = "$PROJECT\diag\scripts\zelda_title_vsram_probe.runtime.lua"
$ROM_BASENAME = [System.IO.Path]::GetFileNameWithoutExtension($Rom)
$ROM_PATH = "$BUILD_DIR\$ROM_BASENAME.md"
$OUTPUT_PATH = "$PROJECT\diag\reports\title_vsram_probe_$ROM_BASENAME.txt"
...
try {
    Write-Host "=== TITLE VSRAM PROBE ===" -ForegroundColor Cyan
    ...
    while (-not $proc.HasExited) {
        ...
        if (Test-Path $OUTPUT_PATH) {
            ...
            if ($content -match 'TITLE VSRAM PROBE COMPLETE') {
