...
$SCRIPT      = "$PROJECT\diag\scripts\zelda_gameplay_interaction_probe.lua"
$TEMP_SCRIPT = "$PROJECT\diag\scripts\zelda_gameplay_interaction_probe.runtime.lua"
$LOCK_PATH   = "$PROJECT\diag\scripts\zelda_gameplay_interaction_probe.lock"
...
$OutputPath = "$REPORTS\gameplay_interaction_probe_$BaseName.txt"
...
try {
    Write-Host "=== GAMEPLAY INTERACTION PROBE ===" -ForegroundColor Cyan
    ...
    while (-not $proc.HasExited) {
        ...
        if ((Get-Date) -gt $deadline) {
            ...
            throw "Timed out after 20 minutes."
        }
    }
    ...
    if ($summarySeen -and (Test-Path $OutputPath)) {
        ...
        Write-Host ""
        $reportLines = Get-Content $OutputPath
        $summaryIndex = [Array]::IndexOf($reportLines, "=== SUMMARY ===")
        if ($summaryIndex -ge 0) {
            $reportLines[$summaryIndex..($reportLines.Length - 1)]
        } else {
            Write-Host "Summary block not found; full report saved to $OutputPath" -ForegroundColor Yellow
        }
    } else {
