...
$SCRIPT    = "$PROJECT\diag\scripts\zelda_room_buffers.lua"
$LOCK_PATH = "$PROJECT\diag\scripts\zelda_room_buffers.lock"

$BaseName = [System.IO.Path]::GetFileNameWithoutExtension($Rom)
$RomPath  = "$BUILD_DIR\$BaseName.md"
$DonePath = "$PROJECT\diag\reports\room_buffers_${BaseName}.done"
$OutPath  = "$PROJECT\diag\reports\room_buffers_${BaseName}.txt"
...
try {
    if (Test-Path $LOCK_PATH) {
        throw "Room buffer dump runner is already active. Remove $LOCK_PATH if stale."
    }
    ...
    if (-not (Test-Path $OutPath)) {
        throw "Room buffer dump did not produce $OutPath"
    }
