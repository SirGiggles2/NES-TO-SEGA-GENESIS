...
$SCRIPT     = "$PROJECT\diag\scripts\zelda_gameplay_state.lua"
$LOCK_PATH  = "$PROJECT\diag\scripts\zelda_gameplay_state.lock"
$REPORTS    = "$PROJECT\diag\reports"
$SEND_ME    = "$REPORTS\Files I Want"

$BaseName   = [System.IO.Path]::GetFileNameWithoutExtension($Rom)
$RomPath    = "$BUILD_DIR\$BaseName.md"
$OutputPath = "$REPORTS\gameplay_state_$BaseName.txt"
...
try {
    Write-Host "=== GAMEPLAY STATE ===" -ForegroundColor Cyan
