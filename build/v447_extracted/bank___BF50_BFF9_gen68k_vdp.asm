...
vec_inc_0x003F60_RESET:  ; orig: vec_inc_0x003F60_RESET:
    NOP                     ; SEI: NES IRQ mask must not block Genesis VBlank/NMI bridge  ; orig: - - - - - - 0x003F60 
    ...
    MOVE.B  D0,D0  ; prep for PPU_WRITE_2000  ; was: - - - - - - 0x003F64 00:BF54: 8D 00 20  STA $2000
    BSR     PPU_WRITE_2000
...
bXX_bra_BF61_loop:  ; orig: bXX_bra_BF61_loop:
    ...
    MOVE.B  #$01,D0  ; orig: - - - - - - 0x003F86 00:BF76: A9 0F     LDA #con_mirroring_H
    BSR     sub_BF98_set_control_register             ; JSR -> BSR  ; orig: - - - - - - 0x003F88 00:BF78: 20 98 BF  JSR
    ...
    MOVE.B  #$07,D0  ; orig: - - - - - - 0x003FA0 00:BF90: A9 07     LDA #con_prg_bank + 
    BSR     sub_BFAC_prg_bankswitch             ; JSR -> BSR  ; orig: - - - - - - 0x003FA2 00:BF92: 20 AC BF  JSR sub_B
...
; bzk garbage
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FD0 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FD8 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FE0 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FE8 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FF0 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x003FF8 
    ; [DIRECTIVE] .BYTE $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x004000 
    ; [DIRECTIVE] .BYTE $FF, $FF  -- needs manual handling  ; orig: - - - - - - 0x004008 00:BFF8: FF        .byte $FF, 
