...
.sync_loop:
    ...
    moveq   #0,D0
    move.b  (A0,D4.w),D0
    move.l  A0,-(A7)            ; FIX v446: save A0 (NES_PAL_TO_BGR555 clobbers it)
    bsr     NES_PAL_TO_BGR555
    move.l  (A7)+,A0            ; FIX v446: restore PPU_PAL_SHADOW pointer
    move.w  D0,($C00000)
    addq.w  #1,D5
    cmpi.w  #$0020,D5
    bne     .sync_loop
    moveq   #0,D0
    move.b  (A0),D0
    move.l  A0,-(A7)            ; FIX v446: save A0
    bsr     NES_PAL_TO_BGR555
    move.l  (A7)+,A0            ; FIX v446: restore A0
    bsr     VDP_SYNC_NES_BACKDROP_COLOR
    movem.l (A7)+,D0-D5/A0
    rts
.sync_frontend_layout:
; FIX v446: Use CORRECT NES palette for frontend, and don't clobber A0
    lea     (PPU_PAL_SHADOW).l,A0
    moveq   #0,D5
.frontend_palette_loop:
    move.w  D5,D4
    lsl.w   #4,D4
    lsl.w   #1,D4
    swap    D4
...
.frontend_color_loop:
    moveq   #0,D4
    tst.w   D3
    beq     .frontend_use_backdrop
    move.w  D5,D4
    lsl.w   #2,D4
    add.w   D3,D4
    bra     .frontend_have_index
.frontend_use_backdrop:
    moveq   #0,D4
.frontend_have_index:
    moveq   #0,D0
    move.b  (A0,D4.w),D0
    bsr     FRONTEND_PAL_LOOKUP     ; FIX v446: correct palette, preserves A0
    move.w  D0,($C00000)
    addq.w  #1,D3
    cmpi.w  #4,D3
    bne     .frontend_color_loop
    addq.w  #1,D5
    cmpi.w  #4,D5
    bne     .frontend_palette_loop
    moveq   #0,D0
    move.b  (A0),D0
    bsr     FRONTEND_PAL_LOOKUP     ; FIX v446: correct palette for backdrop
    bsr     VDP_SYNC_NES_BACKDROP_COLOR
    movem.l (A7)+,D0-D5/A0
    rts
...
; Used ONLY by frontend palette sync to avoid destabilizing gameplay
CORRECT_NES_PALETTE:
;   $00      $01      $02      $03      $04      $05      $06      $07
    DC.W $0444, $0820, $0A22, $0A04, $0806, $0406, $0026, $0024
;   $08      $09      $0A      $0B      $0C      $0D      $0E      $0F
    DC.W $0022, $0040, $0040, $0040, $0440, $0000, $0000, $0000
;   $10      $11      $12      $13      $14      $15      $16      $17
    DC.W $0AAA, $0C62, $0E44, $0E26, $0C28, $062A, $022A, $0048
;   $18      $19      $1A      $1B      $1C      $1D      $1E      $1F
    DC.W $0066, $0084, $0080, $0280, $0880, $0000, $0000, $0000
;   $20      $21      $22      $23      $24      $25      $26      $27
    DC.W $0EEE, $0EA6, $0E88, $0E6C, $0E6E, $0C6E, $068E, $048E
;   $28      $29      $2A      $2B      $2C      $2D      $2E      $2F
    DC.W $00AC, $00CA, $02C6, $08E4, $0CC0, $0666, $0000, $0000
;   $30      $31      $32      $33      $34      $35      $36      $37
    DC.W $0EEE, $0ECA, $0EAA, $0EAC, $0EAE, $0AAE, $0ACE, $0ACE
;   $38      $39      $3A      $3B      $3C      $3D      $3E      $3F
    DC.W $06CE, $06EC, $0AEA, $0CEA, $0EE0, $0000, $0000, $0000
...
NES_PALETTE_DATA:
    DC.W $00A0,$0888,$0000,$0406,$0004,$0024,$0004,$0026
    DC.W $0042,$0040,$0060,$0640,$0444,$0000,$0000,$0000
    DC.W $0AAA,$000E,$0ACE,$060A,$0808,$0808,$0806,$0046
