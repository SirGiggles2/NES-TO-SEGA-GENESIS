...
BASELINE_LABELS = [TABLE_915E, *EXTENDED_ONLY_LABELS, *FULL_DEMO_TEXT, LOGO_PALETTE]

VARIANTS = [
    {"name": "zero_spr_full_layout", "spr_real_bytes": 0x00},
    {"name": "spr_0010_full_layout", "spr_real_bytes": 0x10},
    {"name": "spr_0020_full_layout", "spr_real_bytes": 0x20},
    {"name": "spr_0040_full_layout", "spr_real_bytes": 0x40},
    {"name": "spr_0070_full_layout", "spr_real_bytes": 0x70},
]
...
def extract_spr_data_bytes(text: str) -> list[int]:
    ...
    for line in collect_directive_lines(lines, label_index):
        ...
        if start < 0:
            continue
        payload = line[start + len(marker) : line.find(" -- needs manual handling")]
        values.extend(parse_byte_expr(expr.strip()) for expr in payload.split(","))
