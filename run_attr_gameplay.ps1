...
$SCRIPT    = "$PROJECT\diag\scripts\zelda_attr_gameplay.lua"
$LOCK_PATH = "$PROJECT\diag\scripts\zelda_attr_gameplay.lock"

$BaseName = [System.IO.Path]::GetFileNameWithoutExtension($Rom)
$RomPath  = "$BUILD_DIR\$BaseName.md"
$DonePath = "$PROJECT\diag\reports\attr_gameplay_${BaseName}.done"
$OutPath  = "$PROJECT\diag\reports\attr_gameplay_${BaseName}.txt"
...
try {
    if (Test-Path $LOCK_PATH) {
        throw "Gameplay attr dump runner is already active. Remove $LOCK_PATH if stale."
    }
    ...
    if (-not (Test-Path $OutPath)) {
        throw "Gameplay attr dump did not produce $OutPath"
    }
